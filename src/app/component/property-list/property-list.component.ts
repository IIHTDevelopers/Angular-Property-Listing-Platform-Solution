...
export class PropertyListComponent implements OnInit {
  ...
  constructor(private propertyService: PropertyService, private formBuilder: FormBuilder) {
    this.propertyForm = this.formBuilder.group({
      ...
      rooms: ['', [Validators.required, Validators.min(5), Validators.max(200)]],
  ...
  createProperty(): void {
    ...
    this.propertyService.createProperty(product).subscribe(
  ...
  searchProperties() {
    ...
    this.propertyService.searchProperties(searchQuery).subscribe(
      (data: Property[]) => {
